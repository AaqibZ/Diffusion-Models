...
diffusion outcomes for stochastic structure-oriented diffusion models
...
The following structure-oriented diffusion models are considered:
...
- Monte Carlo simulations: 5 runs per model and seed-set size
- Confidence level: 95%
- Diffusion outcome: final activation count

For each dataset and seed-set size, the diffusion process is repeated
across the Monte Carlo runs. The resulting final activation counts are
used to estimate the mean and variability of the diffusion outcome.
...
Monte Carlo simulations for the structure-oriented diffusion models.
...
structure-oriented diffusion models. The resulting confidence
intervals and error bars are used to supplement the comparative
diffusion analysis presented in the manuscript.
