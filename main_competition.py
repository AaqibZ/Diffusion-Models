# COMPETITION ORIENTED MODELS - Refactored
import gzip
import csv
import random
...
from models_competition import MODELS, WPTM
...
datasets = {
    # Uncomment to include all datasets:
...
def greedy_with_mc(G, k, mc_simulations=MC_SIMULATIONS):
    ...
    Uses WPTM model for seed evaluation.
...
for dataset_name, snapshots in datasets.items():
    print(f"\n{'='*60}")
    print(f"Running competition diffusion on {dataset_name}")
    print(f"{'='*60}")
    ...
    for idx, k in enumerate(K_VALUES):
        ...
        for (m, y), mk, ls in zip(spreads.items(), markers, line_styles):
            line, = ax.plot(range(T_SNAPSHOTS), y, linestyle=ls,
                            marker=mk, linewidth=0.7, markersize=2.5)
            if idx == 0:
    ...
    output_file = f"{dataset_name}_competition_diffusion_plots.pdf"
